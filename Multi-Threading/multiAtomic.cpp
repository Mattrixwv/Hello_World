...
void fibonacciNumberThreaded(int fibNum, std::atomic<uint64_t>* totalFib);
...
int main(){
	...
	std::atomic<uint64_t> fibAtom(0);
	//fibTotal = 0;
	begin = std::chrono::high_resolution_clock::now();
	fibonacciNumberThreaded(fibNum, &fibAtom);
	end = std::chrono::high_resolution_clock::now();
	nanoDur = std::chrono::duration_cast<std::chrono::nanoseconds>(end - begin);
...
void fibonacciNumberThreaded(int fibNum, std::atomic<uint64_t>* totalFib){
	if(fibNum <= 0){
		...
		//Because this is atomic it accomplishes the same thing as the lock
		totalFib += 1;
	}
	else{
		std::thread thr[2];
